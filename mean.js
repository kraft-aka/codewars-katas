...
// There will always be 10 integers and 10 characters. Create a single string with the characters and return
...
function mean(lst) {
  const numsMean =
    lst.filter((i) => !isNaN(i)).reduce((a, b) => a + +b, 0) / 10;
  const str = lst.filter((i) => isNaN(i)).join("");
  return [numsMean, str];
}
