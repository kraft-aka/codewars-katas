...
function countCorrectCharacters(correctWord, guess) {
  ...
  for (let i = 0; i < correctWord.length; i++) {
    if (correctWord[i] == guess[i]) {
      count++;
    } else if (correctWord.length !== guess.length) {
      throw new Error('not the same length');
    }
  }
  return count;
}
...
console.log(countCorrectCharacters('hello', 'heeeo'))
console.log(countCorrectCharacters('hello', 'hello'))
console.log(countCorrectCharacters('hello', 'hee'))