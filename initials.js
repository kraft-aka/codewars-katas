// A person's full name is usually composed of a first name,
// middle name and last name; however in some cultures
// (for example in South India) there may be more than one middle name.

// Write a function that takes the full name of a person, and
// returns the initials, separated by dots ('.').
// The initials must be uppercase.
// The last name of the person must appear in full,
...
function initials(n) {
  ...
  const lastItem = n.slice(-1).join("");
  const lastWord = lastItem[0].toUpperCase() + lastItem.slice(1);
  return (
    n
