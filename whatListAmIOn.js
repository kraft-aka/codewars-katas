...
// 'naughty' or 'nice'. Strings that start with the letters b, f, or k are naughty. Strings that start with the
// letters g, s, or n are nice. Other strings are neither naughty nor nice.
...
function whatListAmIOn(actions) {
  ...
  const naughtyCount = actions.filter((action) =>
    naughtyLetters.includes(action[0].toLowerCase())
  ).length;

  const niceCount = actions.filter((action) =>
    niceLetters.includes(action[0].toLowerCase())
  ).length;
