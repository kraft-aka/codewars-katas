...
// you've come up with the following system: you take the word, cut it in half, and place the first
// half behind the latter. If the word has an uneven number of characters, you leave the middle at its previous place:
...
function reverseByCenter(s) {
  if (s.length % 2 === 0) {
    return s.slice(s.length / 2).concat(s.slice(0, s.length / 2));
  } else {
    return (
      s.slice(Math.ceil(s.length / 2)) +
      s[Math.floor(s.length / 2)].concat(s.slice(0, Math.floor(s.length / 2)))
    );
  }
}
