...
function calculateAge() {
  ...
  if (first < last && last - first == 1)
    return `You are ${last - first} year old.`;
  if (first < last) return `You are ${last - first} years old.`;
  if (first > last && first - last == 1)
    return `You will be born in ${first - last} year.`;
  if (first > last) return `You will be born in ${first - last} years.`;
